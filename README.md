...
- Quadro de Exemplo: https://trello.com/b/uWtvlXO3/teste
- Biblioteca para requisição HTTP: axios

## Roteiro

O aplicativo inicia chamando o componente App que busca todas as listas do quadro do trello.

Após essa busca, ele cria uma lista para cada item retornado, utilizando a função `map` que cria um componente List para cada item.

Cada component List busca seus cards que também são criados utilizando a função `map` e exibidos no componente Card.

Todas as buscas são feitas via protocolo HTTP com o método GET utilizando a biblioteca `axios`. Para outros métodos, basta procurar na documentação.
